...
class FileManager
{
	...
	*/
	public function rename($oldPathname, $newPathname )
	{
		...
		$r = @rename($oldPathname, $newPathname);
		if ($r)  {
			// Success.
			@touch($newPathname);//establecer mTime y aTime del item a now()
			return true;
		} else {
			// Error rename failed.
	...
	*/
	public function createDir($dirname)
	{
		if ( $this->allow_create_folder) {
			...
			$r = @mkdir($this->path.$dir, 0777, true);
			if ( !$r ) 
	...
	public function uploadFile($_FILES_)
	{
		if ( $this->allow_upload ) {
			...
			$res = @move_uploaded_file($_FILES_['file_data']['tmp_name'], $this->path.$_FILES_['file_data']['name']);
			if(!$res){
	...
	{	
		...
		if (is_dir($pathname)) {
			if ( file_exists($pathname.".zip") ) {
				$this->download($pathname.".zip");
				return true ;
			}
	...
	*/
	...
	private function createZipFolder($pathname){
		...
		$nameZip = ROOT_PATH.$pathname.".zip";// .zip *
		$folderPath = ROOT_PATH.$pathname;
